...
pub enum OrderStatus {
    ...
    /// `message_bytes` is hex-encoded raw CCTP message (0x-prefixed, needed for receiveMessage).
    /// `message_hash` is keccak256(message_bytes) — the Circle Iris API lookup key.
    BridgePending { cctp_nonce: u64, message_hash: String, message_bytes: String },
    ...
    /// `message_bytes` is hex-encoded raw CCTP message for the return leg.
    SettlementBridging { cctp_nonce: u64, message_hash: String, message_bytes: String },
    SettlementSwapping,
